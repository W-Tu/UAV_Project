...
#include "custom-aodv-rtable.h"
#include "custom-aodv-rqueue.h"
#include "custom-aodv-packet.h"
#include "custom-aodv-neighbor.h"
#include "custom-aodv-dpd.h"
#include "ns3/node.h"
...
private:
  ...
  /**
   * Mark link to neighbor node as unidirectional for blacklistTimeout
   * \returns the best route based on routing metric
   */
  RoutingTableEntry FindBestRoute();
  ...
  //Flag, set to true when destination receives first RREQ packet
  bool m_destHasReceivedFirstRREQ;
  //Time when the first RREQ was received by destination
  Time m_destReceivedFirstRREQTimeStamp;
  //Flag, set to true when origin receives first RREP packet
  bool m_originHasReceivedFirstRREP;
  //Time when the first RREP was received by origin
  Time m_originReceivedFirstRREPTimeStamp;
  //contains all the routes received by destination within waiting period
  std::vector<RoutingTableEntry> m_rrepRepliesReceivedWithinWaitingPeriod;
};
