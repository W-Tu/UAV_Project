...
#ifndef CUSTOM_OLSR_AGENT_IMPL_H
#define CUSTOMOLSR_AGENT_IMPL_H

#include "custom-olsr-header.h"
#include "ns3/test.h"
#include "custom-olsr-state.h"
#include "custom-olsr-repositories.h"
...
struct RoutingTableEntry
{
  ...
  double routingMetric; //!< Routing metric
...
public:
  ...
  void UpdateRemainingEnergy(double remainingEnergy)
  {
    m_remainingEnergyJoules = remainingEnergy;
  }
...
private:
  ...
  /// Remaining energy left in joules
  double m_remainingEnergyJoules;
...
private:
  ...
  CustomOlsrState m_state;  //!< Internal state with all needed data structs.
  ...
  customOlsr::MessageList m_queuedMessages;
   ...
   */
  void ForwardDefault (customOlsr::MessageHeader olsrMessage,
   ...
   */
  void QueueMessage (const customOlsr::MessageHeader &message, Time delay);
   ...
   */
  void ProcessHello (const customOlsr::MessageHeader &msg,
   ...
   *
   */
  void ProcessTc (const customOlsr::MessageHeader &msg,
                  const Ipv4Address &senderIface);
   ...
   * \param senderIface the address of the interface where the message was sent from.
   */
  void ProcessMid (const customOlsr::MessageHeader &msg,
                   const Ipv4Address &senderIface);
   ...
   *
   */
  void ProcessHna (const customOlsr::MessageHeader &msg,
                   const Ipv4Address &senderIface);
   ...
   */
  void LinkSensing (const customOlsr::MessageHeader &msg,
                    const customOlsr::MessageHeader::Hello &hello,
                    const Ipv4Address &receiverIface,
                    const Ipv4Address &senderIface);
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateNeighborSet (const customOlsr::MessageHeader &msg,
                            const customOlsr::MessageHeader::Hello &hello);
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateTwoHopNeighborSet (const customOlsr::MessageHeader &msg,
                                  const customOlsr::MessageHeader::Hello &hello);
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateMprSelectorSet (const customOlsr::MessageHeader &msg,
                               const customOlsr::MessageHeader::Hello &hello);
   ...
   */
  uint32_t GetRemainingBufferSize();

  /**
   * calcuate routing metric based on hop count buffer size and remaining energy
   * \returns the calcuated routing metric
   */
  double calculateRoutingMetric();
