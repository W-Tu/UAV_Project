 ...
 */
#ifndef CUSTOM_AODVPACKET_H
#define CUSTOM_AODVPACKET_H
...
public:
  ...
  RrepHeader (uint8_t prefixSize = 0, uint8_t hopCount = 0, Ipv4Address dst =
                ...
                Ipv4Address (), Time lifetime = MilliSeconds (0), double routingMetric = 1.0f);
  /**
   * \brief Get the type ID.
   * \return the object TypeId
   */
  ...
  /**
   * \brief Set new routing metric
   * \param newRoutingMetric the new routing metric
   */
  void UpdateRoutingMetricValue(double newRoutingMetric);
  /**
   * \brief gets the current routing metric
   * \returns the current routing metric
   */
  double GetRoutingMetric() const
  {
    return m_routingMetric;
  }
